...
const UserTableRows: React.FC<TableRowsProps> = ({ results, onDelete, totalResults, searchedResults, expandedRow, rowsPerPageDropdown, resultsLength }) => {
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:space-x-2 mb-2">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-6 space-y-2 sm:space-y-0 font-mono">
                    <div className="flex items-center space-x-2">
                        ...
                        {/* <div className={`relative inline-block w-10 h-5 transition duration-200 ease-linear rounded-full cursor-pointer ${isBulkSelect ? 'bg-green-500' : 'bg-red-500'}`}>
                            <span className={`absolute left-1 top-1 w-3 h-3 bg-black rounded-full transition-transform transform ${isBulkSelect ? 'translate-x-5' : 'translate-x-0'}`}></span>
                        </div> */}
                    </div>
